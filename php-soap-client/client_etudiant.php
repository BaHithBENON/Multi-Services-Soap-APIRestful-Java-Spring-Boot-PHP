<?php
    ...
    function create_etudiant($nouveau_nom, $nouveau_prenom, $nouvelle_moyenne) {
        try {
            $soap_client = get_soap_client();
            $arr = new stdClass();
            $arr->nom = $nouveau_nom;
            $arr->prenom = $nouveau_prenom;
            $arr->moyenne = $nouvelle_moyenne;
            return $soap_client->__soapCall("newStudent", array($arr));
        } catch (Exception $exception) {
        ...
        switch ($operation) {
            ...
            case "Créer" : 
                if(isset($_POST['nouveau_nom']) && isset($_POST['nouveau_prenom']) && isset($_POST['nouvelle_moyenne'])) {
                    ...
                    $nouvelleMoyenne = $_POST['nouvelle_moyenne'];
                    $create = create_etudiant($nom, $prenom, $nouvelleMoyenne);
                }
                break;
            default:;
        }
...
<body>
    <div class="box">
        <form action="client_etudiant.php" method="post">
            ...
            <?php 
                if(isset($moyenne) && is_object($moyenne) && property_exists($moyenne, 'return')) {
                    echo "La moyenne de ", $nom, ' ' , $prenom, " est : ", $moyenne->return;
                } else {
                    echo "La moyenne n'est pas disponible,\n vérifiez les noms et prénom de l'étudiant.";
                }
            ?>
            <br />

            <!-- Ajoutez ces éléments à votre formulaire HTML -->
            ...
            <?php if(isset($etudiants)) { ?>
                <h1 style="color:white;">Liste des etudiants : </h1>
